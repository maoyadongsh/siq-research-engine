...
| 美股 / US SEC | Wiki package 在 `/home/maoyd/siq-research-engine/data/wiki/us/...`；SEC parser full json 源头在 `/home/maoyd/siq-research-engine/data/parser-results/us-sec/...` | PostgreSQL importer 读取传入的 `data/wiki/us/.../<package_dir>` evidence package；该 package 内保留 `parser/document_full.json` | `scripts/us-sec/build_sec_evidence_package.py` | `db/imports/import_sec_filing_to_postgres.py` | `db/ddl/010_create_sec_us_schema.sql` | `siq_us`，可由 `SIQ_US_PGDATABASE` 覆盖 | `sec_us` |
| 港股 / HK | Wiki package 在 `/home/maoyd/siq-research-engine/data/wiki/hk/companies/.../reports/...`；PDF parser full json 源头在 `/home/maoyd/siq-research-engine/data/pdf-parser/results/<task_id>` | PostgreSQL importer 读取传入的 `data/wiki/hk/companies/<stock>-<company>/reports/<report_id>` evidence package；该 package 内保留 `document_full.json` / `parser/document_full.json` | `scripts/hk/build_hk_evidence_package.py` | `db/imports/import_hk_evidence_package_to_postgres.py` | `db/ddl/020_create_pdf2md_hk_schema.sql` | `siq_hk`，可由 `SIQ_HK_PGDATABASE` 覆盖 | `pdf2md_hk` |
| 日股 / JP | Wiki package 在 `/home/maoyd/siq-research-engine/data/wiki/jp/companies/.../reports/...`；PDF parser full json 源头在 `/home/maoyd/siq-research-engine/data/pdf-parser/results/<task_id>` | PostgreSQL importer 读取传入的 `data/wiki/jp/companies/<ticker>-<company>/reports/<report_id>` evidence package；该 package 内保留 `document_full.json` / `parser/document_full.json` | `scripts/jp/build_jp_evidence_package.py` | `db/imports/import_jp_evidence_package_to_postgres.py`，底层复用 `db/imports/import_market_xbrl_package_to_postgres.py` | `db/ddl/030_create_edinet_jp_schema.sql` | `siq_jp`，可由 `SIQ_JP_PGDATABASE` 覆盖 | `edinet_jp` |
| 韩股 / KR | Wiki package 在 `/home/maoyd/siq-research-engine/data/wiki/kr/companies/.../reports/...`；PDF parser full json 源头在 `/home/maoyd/siq-research-engine/data/pdf-parser/results/<task_id>` | PostgreSQL importer 读取传入的 `data/wiki/kr/companies/<ticker>-<company>/reports/<report_id>` evidence package；该 package 内保留 `document_full.json` / `parser/document_full.json` | `scripts/kr/build_kr_evidence_package.py` | `db/imports/import_kr_evidence_package_to_postgres.py`，底层复用 `db/imports/import_market_xbrl_package_to_postgres.py` | `db/ddl/040_create_dart_kr_schema.sql` | `siq_kr`，可由 `SIQ_KR_PGDATABASE` 覆盖 | `dart_kr` |
| 欧股 / EU | Wiki package 在 `/home/maoyd/siq-research-engine/data/wiki/eu/companies/.../reports/...`；PDF parser full json 源头在 `/home/maoyd/siq-research-engine/data/pdf-parser/results/<task_id>` | PostgreSQL importer 读取传入的 `data/wiki/eu/companies/<ticker>-<company>/reports/<report_id>` evidence package；该 package 内保留 `document_full.json` / `parser/document_full.json` | PDF: `scripts/eu/build_eu_pdf_evidence_package.py`; ESEF: `scripts/eu/build_eu_esef_evidence_package.py` | `db/imports/import_eu_evidence_package_to_postgres.py` | `db/ddl/050_create_eu_ifrs_schema.sql` | `siq_eu`，可由 `SIQ_EU_PGDATABASE` 覆盖 | `eu_ifrs` |
...
说明：full json 源解析产物确实在 `/home/maoyd/siq-research-engine/data/parser-results` 和 `/home/maoyd/siq-research-engine/data/pdf-parser/results` 两个根目录下。非 A 股市场的 PostgreSQL importer 不直接扫描 `data/pdf-parser/results` 作为主入口；它们通常先由对应 `scripts/<market>/build_*_evidence_package.py` 把解析结果整理到 `data/wiki/<market>/companies/.../reports/<report_id>`，再从这个 evidence package 读取结构化文件入库。
