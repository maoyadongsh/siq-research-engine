...
- 进展补充：`F-004` 继续推进了低耦合前端切片，`SearchDownload.tsx` 的展示派生已下沉到 `features/search-download/display.ts`，`DocumentResultWorkbench.tsx` 的 `pageByNumber` / `previewRelations` / `pageNumbers` / `overlays` / `previewPages` / `previewMarkdownBlocks` 已继续下沉到 `documentResultWorkbenchDerivations.ts`，JSON 预览已拆为独立 `DocumentResultJsonPane.tsx`；`index.css` 已进一步迁出 `auth.css`、`secondary-pages.css`、`agent.css`、`chat.css`，`document-parser/api.ts` 与 `market-parsing/api.ts` 也已从 `export *` 改为显式导出。
...
- 当前建议：先把本轮前端切片按主题提交并保持工作树可回滚；下一轮优先继续 PDF parser quality fallback 覆盖、Agent runtime 只读/纯 helper 迁移，以及前端剩余的少量 barrel 清理。继续避开 `ACTIVE_RUNS` 和本地 queue claim 这两个状态 owner。
