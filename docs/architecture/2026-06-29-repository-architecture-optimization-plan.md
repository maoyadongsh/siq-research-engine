...
- 当前建议：按 0.3 的剩余工作量评估继续推进。下一轮优先继续 PDF parser 的 quality report payload 纯 helper，或继续做 Agent runtime 只读 helper 覆盖；前端只保留 `PDF_CSS` / `DOCUMENT_CSS` 运行时字符串单独评估和必要响应式 smoke，继续避开 `ACTIVE_RUNS`、SSE lifecycle、本地 queue claim 和 Flask `send_file/jsonify` 行为变更。
