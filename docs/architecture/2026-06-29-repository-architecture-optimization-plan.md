...
- `P-002` / `P-001` PDF parser 边界拆分：剩余约 1-3 个小轮次，约 0.5-1 天。
  ...
  2. `document_full` resource / table relation payload 覆盖已补强；open artifact resolver 暂不混入，后续如继续应先抽纯“artifact name 分类/路径/mimetype 决策”helper + 直接单测，再最小接入 route；Flask `send_file/jsonify` 留 app。
