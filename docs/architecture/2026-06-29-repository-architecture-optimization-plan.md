...
- 当前建议：按 0.3 的剩余工作量评估继续推进。下一轮优先做 Agent runtime display/tool-output/context 剩余细边界，或回到 PDF parser 低风险 quality/source-view 补测；继续避开 `ACTIVE_RUNS`、SSE lifecycle、本地 queue claim 和 Flask `send_file/jsonify` 行为变更。
...
3. Agent runtime 窗口：citations/reference、PDF2MD parse-only context、PostgreSQL fallback pure helper 和前端 citation renderer smoke 已补；下一步优先 display/tool-output/context 剩余细边界，真实 attachments/history/local-memory owner 拆分前先补覆盖。
...
1. 优先扩展 `agent_runtime_display.py`、`agent_runtime_tool_output.py` 或 `agent_runtime_context.py` 的剩余细边界，给 loop diagnosis 和 streaming 复用。
