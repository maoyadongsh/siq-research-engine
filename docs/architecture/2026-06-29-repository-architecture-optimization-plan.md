...
- 当前建议：按 0.3 的剩余工作量评估继续推进。下一轮优先做 Agent runtime citations/reference 合并边界补测，其次前端 `PDF_CSS` / `DOCUMENT_CSS` smoke 或 market evidence package 契约边界；继续避开 `ACTIVE_RUNS`、SSE lifecycle、本地 queue claim 和 Flask `send_file/jsonify` 行为变更。
...
3. Agent runtime 窗口：优先做 citations/reference 合并边界补测，或 display/parse-only 细边界补测；真实 attachments/history/local-memory owner 拆分前先补覆盖。
