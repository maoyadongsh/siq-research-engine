...
- 当前建议：按 0.3 的剩余工作量评估继续推进。下一轮优先做 PDF2MD parse-only context 剩余边界，其次做 PostgreSQL fallback pure row/formatter 负路径或前端 citation renderer smoke；继续避开 `ACTIVE_RUNS`、SSE lifecycle、本地 queue claim 和 Flask `send_file/jsonify` 行为变更。
...
6. 本轮追加验证：`cd apps/api && .venv/bin/python -m pytest tests/test_agent_runtime_citations.py tests/test_citation_links.py -q` 通过，52 passed；`cd apps/api && .venv/bin/python -m pytest tests/test_agent_runtime_*.py -q` 通过，174 passed、1 warning；`git diff --check` 通过。
...
3. Agent runtime 窗口：citations/reference 合并边界已补；下一步优先 PDF2MD parse-only context 剩余边界，其次 PostgreSQL fallback pure row/formatter 负路径或前端 citation renderer smoke；真实 attachments/history/local-memory owner 拆分前先补覆盖。
...
1. 优先扩展 `agent_runtime_parse_only.py` 的 PDF2MD parse-only context 剩余边界覆盖，其次补 `agent_runtime_postgres_fallback.py` pure row/formatter 负路径或前端 citation renderer smoke。
2. 后续再继续扩展 `agent_runtime_display.py`、`agent_runtime_tool_output.py`、`agent_runtime_context.py` 或 `agent_runtime_citations.py` 的同类纯函数，给 loop diagnosis 和 streaming 复用。
