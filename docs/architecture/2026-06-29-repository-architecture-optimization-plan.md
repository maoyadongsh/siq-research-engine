...
- 当前建议：按 0.3 的剩余工作量评估继续推进。下一轮优先做 `content_list_enhanced` 财报附注/金额校验 helper、Agent runtime 人效/人均指标展示 helper，或 citations/reference 合并边界补测；继续避开 `ACTIVE_RUNS`、SSE lifecycle、本地 queue claim 和 Flask `send_file/jsonify` 行为变更。
...
```bash
cd apps/pdf-parser && PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -p no:cacheprovider -q  # 244 passed
...
- `cd apps/pdf-parser && PYTHONDONTWRITEBYTECODE=1 python3 -m pytest -p no:cacheprovider -q` 通过，244 passed。
- `cd apps/pdf-parser && python3 -m flask --app app.py routes` 通过。
