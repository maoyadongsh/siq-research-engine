...
通用文档解析是应用中心的核心能力，也是一二级市场共享的材料入口。

| 产品面 | 关系 | 边界 |
| --- | --- | --- |
| 二级市场 | 研报附件、公告补充材料、网页、法规和非标准披露进入统一 artifact | 不替代市场 rules 的财务口径和 package gate |
| 一级市场 | BP、合同、访谈材料、财务模型说明、第三方报告和投委会附件进入 Deal OS 证据层 | 不自动形成投资结论，只提供可引用材料结构 |
| 应用中心 | `/documents`、schema extraction、source map、table relations、向量入库前处理 | 负责“材料编译”，后续入库、召回和 Agent 判断由控制面治理 |
