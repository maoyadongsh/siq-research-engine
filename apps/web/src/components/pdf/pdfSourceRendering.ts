...
function renderBlock(b: PageBlock, index: number, pageNumberValue: number): string {
  ...
  if (type === 'table') {
    ...
    return `<section class="pdf-page-block ${b.is_focus_table ? 'focus-table' : ''}" ${attrs}><div class="pdf-page-block-head"><div><span class="pdf-page-block-type">${escHtml(label)}</span><span class="pdf-page-block-meta">${escHtml(bboxText || '表格解析块')}</span></div>${action}</div><div class="pdf-page-block-tag-row">${tags}</div><div class="pdf-table-wrap pdf-page-table-wrap">${b.table_html ? sanitizeTableHtml(b.table_html) : '<div style="color:#64748b">表格区域，无可用 HTML。</div>'}</div></section>`
  }
  if (type === 'list') {
    const items = (b.list_items || []).map((item: unknown) => `<li>${escHtml(String(item || ''))}</li>`).join('')
    return `<section class="pdf-page-block" ${attrs}><div class="pdf-page-block-head"><div><span class="pdf-page-block-type">列表</span><span class="pdf-page-block-meta">${escHtml(bboxText || '列表解析块')}</span></div></div><ul class="pdf-page-block-list">${items}</ul></section>`
  }
  if (type === 'image') {
    return `<section class="pdf-page-block pdf-page-block-muted" ${attrs}><div class="pdf-page-block-head"><div><span class="pdf-page-block-type">图片</span><span class="pdf-page-block-meta">${escHtml(bboxText || '图片解析块')}</span></div></div><div class="pdf-page-block-text" style="color:#64748b">来源图像：${escHtml(b.image_path || '未提供路径')}</div></section>`
  }
  const headingLike = type === 'header' || Number(b.text_level || 0) > 0
  const label = type === 'header' ? '页眉' : type === 'page_number' ? '页码' : headingLike ? '标题' : '文本'
  return `<section class="pdf-page-block ${type === 'page_number' || type === 'header' ? 'pdf-page-block-muted' : ''}" ${attrs}><div class="pdf-page-block-head"><div><span class="pdf-page-block-type">${escHtml(label)}</span><span class="pdf-page-block-meta">${escHtml(bboxText || '文本解析块')}</span></div></div><div class="pdf-page-block-text ${headingLike ? 'pdf-page-block-heading' : ''}">${escHtml(b.text || b.markdown || ' ')}</div></section>`
}
