...
test('foreground recovery leaves unresolved gaps pending and never enters an infinite rollover loop', async () => {
  ...
  const result = await recoverNativeCaptureAfterForeground({
    ...
    rollover: async () => { rolloverCalls += 1; return {} },
  })
  ...
  const stopped = await recoverNativeCaptureAfterForeground({
    ...
    getCheckpoints: async () => unresolved,
    retryPendingUploads: async () => { throw new Error('must not resume a stopped capture') },
    rollover: async () => { throw new Error('must not roll over a stopped capture') },
  })
  assert.equal(stopped.outcome, 'stopped')
  assert.deepEqual(stopped.steps, ['status', 'checkpoint'])
})

test('server playback switch preserves position and falls back to the opaque local handle', () => {
  const local = { handle: 'capture-asset:capture-1', mediaType: 'audio/wav', durationMs: 10_000 }
  ...
  state = reduceNativePlaybackState(state, { type: 'server_ready', url: '/api/meetings/v1/audio?ticket=opaque' })
  ...
  const failed = reduceNativePlaybackState(state, { type: 'server_switch_failed', message: 'range unavailable' })
  ...
  assert.equal(ready.serverUrl, '/audio?ticket=second')
