...
import { mapProviderFromApi, normalizeNumber, readSetting } from './settings/utils'
...
export default function Settings() {
  ...
  const providerMeta = { local: { title: '本地大模型', desc: '在本机 vLLM 的 Qwen3.6 与 Gemma4 之间切换。', icon: Cpu, iconClass: 'bg-success/10 text-success', panelClass: 'border-primary/20 bg-card' }, cloud: { title: '云端大模型', desc: '复用 Hermes 已有的 Minimax 配置。', icon: Cloud, iconClass: 'bg-primary/10 text-primary', panelClass: 'border-primary/20 bg-card' } } as const
      ...
      <section className="space-y-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-text-muted sm:text-base sm:leading-7">选择本地或云端作为当前调用源。保存后会同步到 SIQ Hermes profiles；本地模型可直接测试 vLLM，云端模型默认优先使用 Hermes Minimax。</p>
          </div>
