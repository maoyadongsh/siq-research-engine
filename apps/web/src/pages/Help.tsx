...
import { InfoCard } from '@/components/research/InfoCard'
...
const dataLocations = [
  ...
  ['下载 PDF', 'data/report-finder/downloads', '搜索下载页保存的公告 PDF，财报解析页可直接选择。'],
  ['PDF 解析结果', 'data/pdf-parser/results', 'Markdown、结构化表格、质量报告和财务抽取产物。'],
  ['聊天历史', 'data/backend/pet.db', '问答助手会话、宠物状态和成就数据。'],
]
    ...
    <div className="secondary-page">
      <PageHeader
        ...
        meta={['下载', '解析', '分析', '核查', '跟踪', '法务'].map((label) => (
          <span key={label} className="secondary-step-chip">{label}</span>
        ))}
      />
        ...
        {entryCards.map((item) => (
          ...
          >
            <ArtifactCard
              ...
              icon={item.icon}
              actions={<ArrowRight className="h-5 w-5 shrink-0 text-text-muted transition-all group-hover:translate-x-1 group-hover:text-primary" />}
            />
          </Link>
        ))}
      </section>
