...
API 地址：`http://localhost:18642/v1`

认证方式：`Authorization: Bearer <HERMES_API_KEY>`
...
```bash
curl -s http://localhost:18642/v1/responses \
...
```bash
curl -s http://localhost:18642/v1/runs \
  -H "Authorization: Bearer <HERMES_API_KEY>" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "hermes-agent",
...
```bash
curl -s http://localhost:18642/v1/runs/run_abc123 \
  -H "Authorization: Bearer <HERMES_API_KEY>"
```
...
```bash
curl -s -N http://localhost:18642/v1/runs/run_abc123/events \
  -H "Authorization: Bearer <HERMES_API_KEY>"
```
...
```bash
curl -s -X POST http://localhost:18642/v1/runs/run_abc123/stop \
  -H "Authorization: Bearer <HERMES_API_KEY>"
```
