...
def test_market_package_build_plan_requires_parser_result_for_hk_jp_kr_and_eu_pdf(tmp_path):
    repo_root = tmp_path / "repo"
    wiki_roots = {"HK": tmp_path / "wiki" / "hk", "JP": tmp_path / "wiki" / "jp", "KR": tmp_path / "wiki" / "kr", "EU": tmp_path / "wiki" / "eu"}
    ...
    for path in (hk_script, jp_script, kr_script, eu_pdf_script, eu_esef_script, hk_source, jp_source, kr_source, eu_pdf_source, parser_result):
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("x", encoding="utf-8")
    ...
    common = {
        "repo_root": repo_root,
        "market_wiki_roots": wiki_roots,
        "market_build_scripts": {"HK": hk_script, "JP": jp_script, "KR": kr_script, "EU": eu_pdf_script},
        "eu_esef_package_build_script": eu_esef_script,
        "safe_download_path": lambda value: (_ for _ in ()).throw(AssertionError("download path should not be used")),
        "adjacent_metadata_path": lambda path: None,
    }
    for market, source in (("HK", hk_source), ("JP", jp_source), ("KR", kr_source), ("EU", eu_pdf_source)):
        try:
            commands.build_market_package_build_plan(
                payload={"source_path": source},
                market=market,
