...
def test_record_financial_llm_provenance_if_needed_records_contract_output(tmp_path):
    ...
    assert record["fact_trust_level"] == "candidate_explanation"
    assert record["output_was_guarded"] is True
    assert record["runtime_provenance"] == {"runtime_target": "host"}


def test_openshell_financial_provenance_uses_terminal_effective_model_not_host_profile(tmp_path):
    profile_dir = tmp_path / "siq_analysis"
    profile_dir.mkdir()
    (profile_dir / "config.yaml").write_text(
    ...
    record = provenance.record_financial_llm_provenance_if_needed(
        ...
        profile_dirs={"siq_analysis": profile_dir},
        model_input={"message": "营业收入是多少？"},
        raw_output="营业收入是 100。",
        stored_output="营业收入是 100。",
        runtime_metadata={
            "configured": {"provider": "openshell-config", "model": "configured-model"},
            "effective": {
                "provider": "openshell-effective",
                "model": "effective-model",
                "authorization": "Bearer must-not-be-persisted",
            },
            "extra": {"api_key": "must-not-be-persisted"},
        },
        runtime_provenance={
        ...
        is_runtime_status_reply=lambda _reply: False,
        needs_financial_evidence_contract=lambda _message, _context: True,
        record_provenance=lambda payload: dict(payload),
    )

    assert record is not None
    assert record["provider"] == "openshell-effective"
    assert record["model"] == "effective-model"
    assert record["runtime_provenance"] == {
        "runtime_target": "openshell",
        "canary_run_id": "canary-0123456789ab",
    }
    assert "host-provider" not in json.dumps(record)
    assert "must-not-be-persisted" not in json.dumps(record)


def test_openshell_financial_provenance_falls_back_to_terminal_configured_not_host_profile(tmp_path):
    profile_dir = tmp_path / "siq_analysis"
    profile_dir.mkdir()
    (profile_dir / "config.yaml").write_text(
        "model:\n  provider: host-provider\n  default: host-model\n",
    ...
    record = provenance.record_financial_llm_provenance_if_needed(
        ...
        runtime_metadata={
            "configured": {"provider": "openshell-config", "model": "configured-model"},
            "effective": {"provider": None, "model": None},
        },
        runtime_provenance={"runtime_target": "openshell"},
        is_runtime_status_reply=lambda _reply: False,
        needs_financial_evidence_contract=lambda _message, _context: True,
        record_provenance=lambda payload: dict(payload),
    )
    ...
    assert record is not None
    assert (record["provider"], record["model"]) == ("openshell-config", "configured-model")


def test_openshell_financial_provenance_without_terminal_runtime_never_uses_host_profile(tmp_path):
    profile_dir = tmp_path / "siq_analysis"
    profile_dir.mkdir()
    (profile_dir / "config.yaml").write_text(
        "model:\n  provider: host-provider\n  default: host-model\n",
    ...
    assert record is not None
    assert (record["provider"], record["model"]) == ("unknown", "unknown")
    assert "host-provider" not in json.dumps(record)


def test_financial_provenance_hash_binds_runtime_and_log_rejects_unsafe_extra(tmp_path):
    common = {
        "provider": "openshell-effective",
        "model": "effective-model",
    ...
    host_record = provenance.build_financial_llm_provenance(
        **common,
        runtime_provenance={"runtime_target": "host"},
    )
    openshell_record = provenance.build_financial_llm_provenance(
        **common,
        runtime_provenance={
            "runtime_target": "openshell",
            "canary_run_id": "canary-0123456789ab",
    ...
    assert host_record["input_hash"] != openshell_record["input_hash"]

    unsafe_record = {
        **openshell_record,
        "provider": "sk-proj-must-not-be-persisted",
        "model": "https://credentials.example/model",
        "authorization": "Bearer must-not-be-persisted",
        "arbitrary_extra": {"api_key": "must-not-be-persisted"},
        "runtime_provenance": {
            **openshell_record["runtime_provenance"],
            "cookie": "session=must-not-be-persisted",
            "base": "http://user:password@127.0.0.1:28651/v1/runs",
        },
    }
    log_path = tmp_path / "financial_llm_provenance.jsonl"
    stored = provenance.record_financial_llm_provenance(unsafe_record, log_path=log_path)
    serialized = log_path.read_text(encoding="utf-8")

    assert stored["provider"] == "unknown"
    assert stored["model"] == "unknown"
    assert stored["runtime_provenance"] == {
        "runtime_target": "openshell",
        "canary_run_id": "canary-0123456789ab",
    }
    assert "authorization" not in stored
    assert "arbitrary_extra" not in stored
