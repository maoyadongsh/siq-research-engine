...
DOCUMENT_FULL_SCHEMA_VERSION = 1
CONTENT_LIST_ENHANCED_SCHEMA_VERSION = 8
...
def _get_task(task_id):
    ...
    try:
        ...
        # fallback: 短前缀匹配（evidence_index 用 8 字符短 UUID）
        rows = conn.execute(
            "SELECT * FROM tasks WHERE task_id LIKE ? ORDER BY created_at DESC LIMIT 1",
            (task_id + "%",),
        ).fetchall()
        if rows:
...
def _markdown_page_index(markdown):
    text = str(markdown or "")
...
def _build_document_full_json(task, markdown, enhanced, quality_report, financial_data=None, financial_checks=None):
    ...
    return {
        ...
        "markdown": {
            ...
            "pages": _markdown_page_index(markdown),
        },
        "content_list": content_list,
