...
export default function ChatPage() {
  ...
  return (
    <div className="premium-shell flex h-[calc(100vh-96px)] flex-col overflow-hidden rounded-[30px]">
      {/* Header */}
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {quickQuestions.map((q) => (
                <button
                  ...
                  className="premium-chip text-primary transition-colors hover:border-primary/20 hover:bg-primary/10"
                >
                  {q}
                </button>
              ))}
        ...
        <div className="mx-auto max-w-3xl space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              <div
                className={`rounded-[22px] px-4 py-3 text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'max-w-[86%]' : 'max-w-[96%]'} ${
                  msg.role === 'user'
