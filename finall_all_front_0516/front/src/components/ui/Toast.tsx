...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const toast = useCallback((input: ToastInput) => {
    ...
    window.setTimeout(() => remove(id), input.duration ?? 3200)
  }, [remove])
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div className="fixed right-4 top-24 z-[90] flex w-[min(360px,calc(100vw-2rem))] flex-col gap-3">
        {items.map((item) => {
          const meta = styles[item.type]
          const Icon = meta.icon
          return (
            <div key={item.id} className="rounded-2xl border border-border bg-card/96 p-4 shadow-2xl shadow-slate-900/12 backdrop-blur-xl">
              <div className="flex items-start gap-3">
                ...
                <button className="rounded-lg p-1 text-text-muted hover:bg-bg hover:text-text" onClick={() => remove(item.id)} aria-label="关闭提示">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
