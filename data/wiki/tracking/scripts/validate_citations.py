...
def _has_page_ref(ref: dict[str, Any]) -> bool:
    return bool(
        ref.get("pdf_page")
...
def _validate_tracking_items(tracking_dir: Path) -> list[dict[str, str]]:
    ...
    for item in parsed.get("items", []):
        ...
        if not refs:
            ...
            continue
        if not any(_has_page_ref(ref) for ref in refs if isinstance(ref, dict)):
            issues.append({"file": str(path), "item": item.get("id", ""), "issue": "source_refs 缺少 PDF 页码或可打开来源链接"})
    return issues
...
def _validate_latest_metrics(tracking_dir: Path) -> list[dict[str, str]]:
    ...
    for metric in metrics:
        ...
        if not refs:
            ...
            continue
        if not any(_has_page_ref(ref) for ref in refs if isinstance(ref, dict)):
            issues.append({"file": str(path), "metric": metric.get("canonical_name", ""), "issue": "指标 source_refs 缺少 PDF 页码或可打开来源链接"})
    return issues
...
def _validate_alerts(tracking_dir: Path) -> list[dict[str, str]]:
    ...
    for path in latest_alerts:
        ...
        if json_block:
                ...
                if category in {"异常指标", "财务承诺", "异常变动", "metric"}:
                    ...
                    if not any(_has_page_ref(ref) for ref in refs if isinstance(ref, dict)):
                        issues.append({
                            "file": str(path),
                            "alert_id": entry.get("id", ""),
                            "issue": "alert source_refs 缺少 PDF 页码或可打开来源链接",
                        })
        else:
            ...
            if re.search(r"(同比|环比|本期|上期).{0,40}([-+]?\d+(\.\d+)?\s*(亿元|万元|%|倍|个百分点))", content):
...
def _validate_updates(tracking_dir: Path) -> list[dict[str, str]]:
    ...
    for path in sorted(updates_dir.glob("*.md")):
        ...
        if re.search(r"[-+]?\d+(\.\d+)?\s*(亿元|万元|%|倍)", content):
