...
REPORT_NAME_PATTERN = re.compile(
    r"^([A-Za-z0-9][A-Za-z0-9._-]{0,31})-(.+)-跟踪报告-(\d{4}-\d{2}-\d{2})\.html$"
)
REPORT_MANIFEST_NAME = "report_manifest.json"
LATEST_REPORT_LINK = "latest.html"

# 市场展示代码格式。正式身份由 ResearchIdentity 决定，ticker 只用于展示和文件名。
SECURITY_CODE_PATTERN = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,31}$")
...
def get_company_dir(stock_code: str, company_name: str) -> str:
    """获取公司主目录路径"""
    return str(company_dir_path(WIKI_BASE, stock_code, company_name))
...
def is_v2_tracking_artifact_html(tracking_dir: str | os.PathLike[str], filename: str) -> bool:
    """Identify a canonical AgentArtifactV2 HTML that cleanup must preserve."""
    html_path = Path(tracking_dir) / filename
    if html_path.suffix.lower() != ".html":
        return False
    sidecar_path = html_path.with_suffix(".artifact.json")
    ...
    try:
        ...
        return False
    return (
        payload.get("schema_version") == "siq_agent_artifact_v2"
        and payload.get("artifact_type") == "tracking"
        and payload.get("html_file") == filename
    )
...
def write_report_manifest(stock_code: str, company_name: str, report_path: str) -> str:
    ...
    manifest = {
        ...
        # Keep the manifest portable. Consumers resolve this basename inside
        # the approved tracking directory instead of persisting a host path.
        "latest_report_path": report_name,
...
def validate_single_report_policy(stock_code: str, company_name: str, date: Optional[str] = None) -> bool:
    ...
    for f in os.listdir(tracking_dir):
        if not f.endswith('.html') or f == LATEST_REPORT_LINK:
            continue
        if is_v2_tracking_artifact_html(tracking_dir, f):
            continue
...
def check_analysis_completed(stock_code: str, company_name: str) -> bool:
    ...
    """
    company_dir = Path(get_company_dir(stock_code, company_name)).resolve()
    analysis_dir = company_dir / "analysis"
    if not os.path.exists(analysis_dir):
        return False

    explicit_artifact = str(os.environ.get("SIQ_TRACKING_ANALYSIS_ARTIFACT") or "").strip()
    expected_identity_raw = str(os.environ.get("SIQ_TRACKING_RESEARCH_IDENTITY") or "").strip()
    if explicit_artifact:
        artifact_path = Path(explicit_artifact).expanduser().resolve()
        try:
            artifact_path.relative_to(analysis_dir.resolve())
        except ValueError:
            return False
        if not artifact_path.is_file() or artifact_path.name == "README.md":
            return False
        if not expected_identity_raw:
            return True
        try:
            expected_identity = json.loads(expected_identity_raw)
        except json.JSONDecodeError:
            return False
        sidecar_candidates = [
            artifact_path.with_suffix(".artifact.json"),
            artifact_path.parent / f"{artifact_path.stem}.artifact.json",
        ]
        ...
        try:
            ...
            return False
        target = payload.get("research_target") if isinstance(payload, dict) else None
        actual_identity = target.get("research_identity") if isinstance(target, dict) else None
        if not isinstance(actual_identity, dict):
            actual_identity = payload.get("research_identity") if isinstance(payload, dict) else None
        fields = ("market", "company_id", "filing_id", "parse_run_id")
        return isinstance(actual_identity, dict) and all(
            str(actual_identity.get(field) or "") == str(expected_identity.get(field) or "")
            for field in fields
        )

    # CN legacy compatibility: README and directories do not count as a baseline.
    try:
        return any(
            path.is_file()
            and path.name != "README.md"
            and path.suffix.lower() in {".html", ".md", ".json"}
            and not path.name.endswith(".artifact.json")
...
def validate_stock_code(stock_code: str) -> bool:
    """校验市场展示代码格式；正式公司身份不依赖该值。"""
    return bool(SECURITY_CODE_PATTERN.fullmatch(str(stock_code or "").strip()))
...
def preflight_check(stock_code: str, company_name: str) -> Tuple[bool, List[str]]:
    ...
    # 检查展示代码格式。AAPL、BRK.B、00005 等均为有效值。
    if not validate_stock_code(stock_code):
        errors.append(f"证券代码格式错误: {stock_code}")
