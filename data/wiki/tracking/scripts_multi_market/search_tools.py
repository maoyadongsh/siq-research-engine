...
MARKET_SEARCH_POLICY = {
    "CN": {"news": "最新公告 新闻 巨潮资讯 交易所", "risk": "监管 处罚 问询 立案调查"},
    "HK": {"news": "latest announcement HKEX news", "risk": "HKEX SFC regulatory investigation litigation"},
    "US": {"news": "latest filing SEC EDGAR company news", "risk": "SEC investigation litigation regulatory risk"},
    "JP": {"news": "latest filing EDINET TDnet company news", "risk": "FSA JPX regulatory investigation risk"},
    "KR": {"news": "latest filing DART KRX company news", "risk": "FSS KRX regulatory investigation risk"},
    "EU": {"news": "latest regulatory announcement annual report exchange news", "risk": "regulatory investigation litigation ESMA risk"},
}


def _market_search_terms(kind: str) -> str:
    market = str(os.environ.get("SIQ_TRACKING_MARKET") or "CN").strip().upper()
    policy = MARKET_SEARCH_POLICY.get(market, MARKET_SEARCH_POLICY["CN"])
    return policy[kind]
...
class SearchTools:
    ...
    def search_company_news(
        ...
        Returns:
            搜索结果字典
        """
        query = f"{company_name} {stock_code} {_market_search_terms('news')}"
    ...
    def search_company_risks(
        ...
        Returns:
            搜索结果字典
        """
        query = f"{company_name} {stock_code} {_market_search_terms('risk')}"
        return self.search(
            query=query,
            backend="tavily",
            max_results=max_results,
    ...
    def search_industry_trends(
        ...
        Returns:
            搜索结果字典
        """
        query = f"{industry} 行业政策 趋势 {datetime.now().year}"
        return self.search(
            query=query,
            backend="exa",
            max_results=max_results,
