...
def markdown_to_html(md: str) -> str:
    ...
    # 代码块先替换为占位节点，避免 YAML/JSON 中的列表、表格和行内代码
    # 被后续的轻量 Markdown 规则再次解释。
    code_blocks: List[str] = []

    def replace_fenced_code(match: re.Match) -> str:
        language = re.sub(r"[^a-z0-9_-]", "", (match.group(1) or "").lower())
        if language in {"json", "yaml", "yml"}:
            code_blocks.append(
                '<p class="structured-data-note">完整结构化数据保留在跟踪工作文件中，'
                'HTML 报告不重复嵌入内部字段和证据标识。</p>'
            )
            return f'<div data-siq-code-block="{len(code_blocks) - 1}"></div>'
        class_names = "code-block" + (f" {language}" if language else "")
        summary = "查看完整技术明细"
        content = match.group(2)
        if len(content) > 8_000:
            content = content[:8_000].rstrip() + "\n... 技术明细已截断，完整内容见工作文件。\n"
        code_blocks.append(
            '<details class="raw-technical-details">'
            f'<summary>{summary}</summary>'
            f'<pre class="{class_names}"><code>{content}</code></pre>'
            '</details>'
        )
        return f'<div data-siq-code-block="{len(code_blocks) - 1}"></div>'
    ...
    for index, block in enumerate(code_blocks):
        html = html.replace(f'<div data-siq-code-block="{index}"></div>', block)
...
def markdown_summary_count(content: str, label: str) -> Optional[int]:
    match = re.search(rf'\*\*{re.escape(label)}\*\*:\s*(\d+)\s*项', content or "")
...
def generate_html_report(
    ...
    sentiment_state = str(os.environ.get("SIQ_TRACKING_SENTIMENT_STATUS") or "").strip().lower()
    sentiment_reason = str(os.environ.get("SIQ_TRACKING_SENTIMENT_REASON") or "").strip()
    sentiment_content = read_markdown_file(os.path.join(sentiment_dir, sentiment_files[0])) if sentiment_files else ""
    if sentiment_state == "skipped":
        sentiment_html = "<p><strong>本次未执行舆情检索。</strong> 请求显式跳过该模块；该状态不代表已完成检索，也不能用于判断舆情数量、方向或风险程度。</p>"
        sentiment_badge = "本次未执行"
        sentiment_stat = "未执行"
    elif sentiment_state in {"unavailable", "failed"}:
        reason_copy = f"（{esc(sentiment_reason)}）" if sentiment_reason else ""
        sentiment_html = f"<p><strong>本次舆情来源不可用{reason_copy}。</strong> 未取得真实来源；该状态不代表已完成检索，也不能用于判断舆情数量、方向或风险程度。</p>"
        sentiment_badge = "来源不可用"
        sentiment_stat = "不可用"
    elif sentiment_content:
        sentiment_html = markdown_to_html(sentiment_content)
        sentiment_badge = file_badge(sentiment_files)
        sentiment_stat = str(len(sentiment_files))
    else:
        sentiment_html = "<p><strong>本次未生成舆情结果。</strong> 执行状态未记录；该状态不代表已完成检索，也不能用于判断舆情数量、方向或风险程度。</p>"
        sentiment_badge = "未生成"
        sentiment_stat = "未生成"
    ...
    tracked_metric_count = markdown_summary_count(metrics_content, "追踪指标数") or 0
    comparable_metric_count = markdown_summary_count(metrics_content, "可比较指标")
    metrics_insufficient = (
        not metrics_content
        or tracked_metric_count == 0
        or comparable_metric_count == 0
    )
    ...
    if alerts_content:
        alerts_html = markdown_to_html(alerts_content)
    elif metrics_insufficient:
        alerts_html = "<p><strong>当前没有已触发预警，但指标数据不足，不能据此排除风险。</strong></p>"
    else:
        alerts_html = "<p><em>当前无活跃预警</em></p>"
    ...
    if alert_count > 0:
        ...
        status_tone = "watch"
        status_label = "数据不足"
        status_message = "结构化指标缺失或缺少可比期间，当前无法判定趋势，也不能据此得出无预警结论。"
    elif sentiment_state in {"skipped", "unavailable", "failed"}:
        status_tone = "watch"
        status_label = "部分模块未执行"
        status_message = "财务指标已完成本地跟踪，但本次舆情检索未执行或来源不可用；报告保持降级状态。"
    else:
        status_tone = "steady"
        status_label = "常规跟踪"
    ...
    if attention_items:
        ...
        attention_html = '<div class="empty-state">指标数据不足，当前无法判断是否存在高优先级变化。</div>'
    else:
...
.section-content .raw-technical-details {
  margin: 14px 0;
  border: 1px solid var(--line);
  border-radius: 8px;
  background: #fbfcfd;
}
.section-content .raw-technical-details > summary {
  cursor: pointer;
  padding: 12px 16px;
  color: var(--accent-blue);
  font-weight: 700;
}
.section-content .raw-technical-details[open] > summary {
  border-bottom: 1px solid var(--line);
}
.section-content pre {
  max-height: 420px;
    ...
    <main class="page">
        <header class="hero">
            ...
            <div class="stats-grid" aria-label="跟踪摘要">
                ...
                <div class="stat-card yellow"><span>追踪指标</span><b>{esc(tracked_metric_count)}</b></div>
                <div class="stat-card {'red' if alert_count > 0 else 'green'}"><span>活跃预警</span><b>{esc(alert_count)}</b></div>
                <div class="stat-card purple"><span>舆情状态</span><b>{esc(sentiment_stat)}</b></div>
            </div>
        ...
        <section class="report-section" id="sentiment">
            <button class="section-header" type="button" onclick="toggleSection(this)" aria-expanded="true">
                ...
                <span class="section-actions">
                    <span class="badge">{esc(sentiment_badge)}</span>
                    <span class="toggle-arrow" aria-hidden="true">▼</span>
                </span>
            </button>
            <div class="section-content">
