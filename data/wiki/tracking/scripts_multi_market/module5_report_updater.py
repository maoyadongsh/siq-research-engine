...
def _checkpoint_metadata() -> dict:
    try:
        identity = json.loads(os.environ.get("SIQ_TRACKING_RESEARCH_IDENTITY", ""))
    except json.JSONDecodeError:
        identity = None
    try:
        previous = json.loads(os.environ.get("SIQ_TRACKING_PREVIOUS_CHECKPOINT", "null"))
    except json.JSONDecodeError:
        previous = None
    if not isinstance(identity, dict):
        return {}
    return {
        "schema_version": "siq_tracking_checkpoint_v1",
...
def generate_update_section(tracking_data: dict, update_date: str) -> str:
    ...
    # 2. 最新舆情摘要。本轮未执行或来源不可用时不得复用历史日报冒充当前结果。
    sentiment_state = str(os.environ.get("SIQ_TRACKING_SENTIMENT_STATUS") or "").strip().lower()
    if sentiment_state == "skipped":
        section += "### 最新舆情摘要\n\n"
        section += "本次未执行舆情检索；该状态不代表已完成检索，也不能用于判断舆情数量、方向或风险程度。\n\n"
    elif sentiment_state in {"unavailable", "failed"}:
        section += "### 最新舆情摘要\n\n"
        section += "本次真实舆情来源不可用；该状态不代表已完成检索，也不能用于判断舆情数量、方向或风险程度。\n\n"
    elif tracking_data.get("sentiment"):
        section += "### 最新舆情摘要\n\n"
    ...
    if tracking_data.get("metrics"):
        ...
        comparable_match = re.search(r'\*\*可比较指标\*\*:\s*(\d+)\s*项', content)
        tracked_match = re.search(r'\*\*追踪指标数\*\*:\s*(\d+)\s*项', content)
        metrics_insufficient = (
            not tracked_match
            or int(tracked_match.group(1)) == 0
            or (comparable_match is not None and int(comparable_match.group(1)) == 0)
        )
        # 提取关键指标表
        lines = content.split('\n')
        in_table = False
    ...
    if tracking_data.get("alerts"):
        ...
        section += "### 预警状态\n\n"
        if metrics_insufficient:
            section += "⚠️ 当前没有已触发预警，但指标数据不足，不能据此排除风险。\n\n"
        else:
            section += "✅ 当前无活跃预警\n\n"
