"""Wiki file serving router – serves /home/maoyd/wiki/companies/ files."""
import json
import os
import glob
...
def _safe_path(requested: str) -> str:
    """Resolve and validate that the requested path stays inside WIKI_ROOT."""
    full = os.path.normpath(os.path.join(WIKI_ROOT, requested))
    if not full.startswith(os.path.normpath(WIKI_ROOT)):
...
def list_recent_results(limit: int = 8):
...
def search_generated_reports(q: str = "", limit: int = 10):
    query = q.strip().lower()
...
def list_reports(company_dir: str):
...
def list_factchecks(company_dir: str):
    safe = _safe_path(os.path.join("companies", company_dir))
...
def list_trackings(company_dir: str):
    safe = _safe_path(os.path.join("companies", company_dir))
...
def list_legals(company_dir: str):
    safe = _safe_path(os.path.join("companies", company_dir))
...
@router.delete("/companies/{company_dir}/{result_type}/{filename}")
def delete_generated_report(company_dir: str, result_type: str, filename: str):
