...
- 半衰期：动态记忆默认按时间衰减，近期经验自然优先，旧偏好不会永久污染新任务。
...
最新状态已经从“长期驻留手工灰度”升级为：有效公司上下文触发自动创建公司级沙箱，同一前端对话内同公司复用沙箱代际，切换公司生成隔离代际，请求结束后租约归零，空闲 TTL 后自动销毁。当前可准确表述为：**针对 SIQ 投研业务合同定制的、非 NemoClaw 路径的原生 NVIDIA OpenShell + Hermes 演示/灰度控制面**。它是项目技术壁垒之一，但正式生产切流必须等待正式 A/B、人工安全评审和 `check_v06_completion.py` 从 `NO_GO` 进入 `GO`。
...
| OpenShell 发布门禁 | `python3 scripts/openshell/check_v06_completion.py --json` | 当前正式生产门禁仍为 `NO_GO`；只在正式证据、A/B、质量门禁和人工评审齐全后才能 GO |
