...
截至 2026-07-06，SIQ 已从“多服务技术验证”推进到“可售卖样板闭环”阶段。当前最重要的产品样板是港股二级市场研究闭环，同时保留美股 SEC、A 股 PDF、欧股 ESEF、日股 EDINET、韩股 DART 和通用文档解析能力。

| 领域 | 当前状态 | 说明 |
| --- | --- | --- |
| 商业 MVP | 港股二级市场闭环 | HKEX / 已下载年报 -> PDF parser -> Wiki evidence package -> quality gates -> PostgreSQL / vector dry-run |
...
| 评测体系 | 静态样本 + E2E + eval run | 新增 `datasets/market_ingestion`、`artifacts/eval-runs` 和 market ingestion 指标口径 |
| 架构治理 | owner boundary 收口中 | package action、parser route payload、agent runtime 大文件继续拆分，但不阻塞 MVP |
| 智能体协作 | 二级市场 + 一级市场并行 | 二级市场助手/分析/核查/跟踪/法务已成体系；一级市场 Deal OS / IC profiles 正在产品化 |
...
首个可售卖样板收敛为港股年度报告解析闭环：

```text
HKEX / 已下载港股 PDF
  -> PDF parser
  -> Wiki evidence package
  -> quality gates
  -> PostgreSQL import / vector dry-run
  -> Web 工作台复核与 force override 审计
```

MVP 的核心不是“能解析一个 PDF”，而是证明以下能力可重复运行：

- 官方来源明确：样本必须来自 HKEX 或显式 official source tier。
- evidence package 可定位：package 能被 API 和前端稳定读取。
- 质量状态可见：证据覆盖、三大表覆盖、hash、parser/rule warning 在同页展示。
- 风险动作受控：warning/fail package 未确认不得入库或生成检索层。
- 评测可量化：`official_source_hit_rate`、`parser_success_rate`、`evidence_coverage_ratio`、`statement_coverage`、`bridge_check_pass_rate` 等指标可持续跟踪。

相关样本和运行记录：

- `datasets/market_ingestion/secondary_market_mvp_cases.json`
- `artifacts/eval-runs/2026-07-06-secondary-market-mvp/`
- `apps/web/e2e/tests/secondary-market-mvp-flow.spec.ts`
