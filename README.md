...
| OpenShell 安全层 | v0.6 基础设施与离线验证完成，业务切流保持 NO_GO | 项目 gateway、host brokers、service/broker preflight、BYOC smoke、provider-independent deny-all probe、transaction v2、编译配置 hash gate、host Publisher 和回滚链已验证；正式 `siq_analysis` sandbox/A-B 仍未完成，Host Hermes 继续默认 |
...
SIQ_START_OPENSHELL_BROKERS=1 ./start_all.sh
...
| `SIQ_HERMES_RUNTIME` | `host` | 当前只允许 `host`；正式 A/B 通过前拒绝自动切换 OpenShell Hermes |
| `SIQ_START_OPENSHELL_GATEWAY` | `1` | 随项目启动或复用项目专用 OpenShell gateway |
| `SIQ_START_OPENSHELL_BROKERS` | `auto` | reader secret 存在时启动/复用 `18792/18793`，缺失时不影响 host runtime |
