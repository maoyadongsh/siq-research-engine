...
截至 2026-07-21，项目已从多服务技术验证进入“可演示、可复核、可扩展”的平台化阶段。

2026-07-21 的本轮交付收口继续核对了实际代码、模型启动脚本、DGX Spark 运行状态、OpenShell 完成度门禁、主要数据服务和干净 CI runner 合同；动态状态以各自标注的采样日期为准。
...
| 测试资产 | 当前数量 | 覆盖重点 |
| --- | ---: | --- |
| Python 测试文件 | 493 | API、parser、market services、contracts、db imports、Hermes、OpenShell、model-services |
| TypeScript / Playwright / Node 测试文件 | 115 | Web 路由、工作台交互、meeting 前端协议、E2E smoke、iOS capture 合同 |
| Shell 脚本 | 77 | 启动、运维、OpenShell、Hermes、模型服务和 smoke 入口 |
| OpenShell 专项回归 | 2026-07-21 本机执行 `1126 passed` | 运行面选择、资源池绑定、租约、范围自动创建、对话沙箱代际、TTL、恢复、Host 回退和运行来源回执 |

2026-07-21 采用 `rg --files` 并排除 `data/var/artifacts/runtimes/.venv/node_modules` 后计数：Python test 文件 `493`、TypeScript/Playwright test/spec 文件 `115`、`.sh` 文件 `77`。数量会随分支合并变化，质量判断仍应以实际测试命令与通过结果为准，而不是以文件数替代覆盖率。
...
| OpenShell 专项回归 | `python3 -m pytest -q scripts/openshell/tests` | 2026-07-21 本机执行 `1126 passed`，覆盖运行面选择、资源池、租约、自动创建、对话沙箱代际、TTL 和恢复 |
...
2026-07-21 使用与 GitHub Actions 相同的临时 SQLite、data、runtime、artifacts 和仓库内 Wiki fixture 完成了以下收口验证：

- API 非慢速/非网络测试：`2919 passed, 5 skipped, 3 deselected`。
- OpenShell 离线测试：`1126 passed`。
- Market ingestion helper suite：`484 passed`；`packages/market-contracts`：`37 passed`。
- Ruff、Python 编译和 `git diff --check` 通过。

API CI 不再依赖开发机的完整 Wiki 或运行态数据库。工作流将 `SIQ_WIKI_ROOT` 指向仓库内的 [脱敏公开披露 fixture](datasets/fixtures/api_ci/README.md)，其中只保留测试需要的 metrics、表格窗口、页码和身份映射，不包含原始 PDF、用户上传、运行数据库、模型回答或密钥。财务 QA 的 7 个跨市场 `wiki-static` case 也已绑定到仓库内的 [确定性 benchmark 快照](datasets/eval/financial_qa_benchmark/v1/README.md)，便于评委在干净 runner 上复核。
...
| `SIQ_HERMES_RUNTIME` | `host` | 仓库和启动脚本的全局环境基线；单个 profile 可由运行态选择文件热覆盖 |
...
`SIQ_HERMES_RUNTIME=host` 描述的是可复现的仓库基线，不等于每台已部署机器的实时路由。2026-07-21 本机采样中，未纳入 Git 的 `var/openshell/runtime-selection/siq-analysis.json` 已设置 `profile=siq_analysis`、`target=openshell`、`session_mode=all`，后端 `/health` 同时返回 `openshell_recovery.ready=true`；因此新分析请求实际进入 OpenShell，而其他未匹配 profile/scope 仍保持 Host。该运行态文件默认被 Git 忽略，不构成正式生产发布证据，也不改变 `check_v06_completion.py` 的 `NO_GO` 结论。
