...
| 能力 | 不能只看 | 应同时评测 |
| --- | --- | --- |
| 解析 | OCR 字符正确率 | 章节/表格/页码/bbox/source map、财务字段、quality gate |
| 问答 | 语言相似度 | ResearchIdentity、引用支持度、数值/单位/期间、拒答与缺口 |
| 财务 | 最终数字相等 | 输入 evidence、公式、Decimal 重算、负基数/除零/外汇状态 |
| 记忆 | 召回命中 | ACL、scope、时间衰减、全量召回、跨公司/parse run 隔离 |
| 图片 | 描述流畅 | 文字/数字/表格/图表定位、不确定性与证据交叉验证 |
| 语音/会议 | 单句转写 | CER、延迟、重连、stable segment、DER/voiceprint、隐私删除 |
| OpenShell | 沙箱存活 | 文件/网络/Provider/Broker 边界、A/B 质量、恢复与回滚 |

Mock 和空 fixture 只验证协议 fail-closed 行为，必须在报告中与真实模型/真实授权样本结果分开。
