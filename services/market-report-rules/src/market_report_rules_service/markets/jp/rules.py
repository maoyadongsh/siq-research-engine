...
JP_LABEL_RULES: tuple[MetricRule, ...] = (
    ...
    MetricRule("gross_profit", StatementType.INCOME_STATEMENT, ("売上総利益", "営業総利益", "gross profit")),
    ...
    MetricRule("total_profit", StatementType.INCOME_STATEMENT, ("税引前利益", "税金等調整前当期純利益", "経常利益", "profit before tax", "profit before income taxes", "income before income taxes", "income before income tax", "ordinary income")),
    MetricRule("income_tax_expense", StatementType.INCOME_STATEMENT, ("法人所得税費用", "法人税等合計", "法人税等", "income tax")),
    MetricRule("net_profit", StatementType.INCOME_STATEMENT, ("当期利益", "当期純利益", "profit for the year", "net income"), 10),
    ...
    MetricRule("nci_profit", StatementType.INCOME_STATEMENT, ("非支配持分に帰属する当期利益", "非支配株主に帰属する当期純利益", "non-controlling interests")),
    MetricRule("total_assets", StatementType.BALANCE_SHEET, ("資産合計", "総資産", "total assets"), 10),
    MetricRule("current_assets", StatementType.BALANCE_SHEET, ("流動資産合計", "流動資産", "current assets")),
    MetricRule("non_current_assets", StatementType.BALANCE_SHEET, ("非流動資産合計", "固定資産合計", "非流動資産", "固定資産", "non-current assets")),
    MetricRule("total_liabilities", StatementType.BALANCE_SHEET, ("負債合計", "total liabilities"), 10),
    MetricRule("current_liabilities", StatementType.BALANCE_SHEET, ("流動負債合計", "流動負債", "current liabilities")),
    MetricRule("non_current_liabilities", StatementType.BALANCE_SHEET, ("非流動負債合計", "固定負債合計", "非流動負債", "固定負債", "non-current liabilities")),
    MetricRule("total_equity", StatementType.BALANCE_SHEET, ("資本合計", "純資産", "純資産合計", "total equity", "net assets"), 10),
    ...
    MetricRule("total_liabilities_and_equity", StatementType.BALANCE_SHEET, ("負債及び資本合計", "負債及び純資産合計", "負債純資産合計", "total liabilities and equity")),
    ...
    MetricRule("operating_cash_flow_net", StatementType.CASH_FLOW_STATEMENT, ("営業活動によるキャッシュフロー", "営業活動によるキャッシュ・フロー", "営業活動に関するキャッシュフロー", "営業活動に関するキャッシュ・フロー", "営業活動による現金及び現金同等物の増加", "営業活動による現金及び現金同等物の減少", "net cash provided by operating activities", "net cash provided by used in operating activities", "net cash provided by (used in) operating activities", "net cash generated by operating activities", "cash generated by operating activities", "cash flows from operating activities"), 10),
    MetricRule("investing_cash_flow_net", StatementType.CASH_FLOW_STATEMENT, ("投資活動によるキャッシュフロー", "投資活動によるキャッシュ・フロー", "投資活動に関するキャッシュフロー", "投資活動に関するキャッシュ・フロー", "投資活動による現金及び現金同等物の増加", "投資活動による現金及び現金同等物の減少", "net cash used in investing activities", "net cash provided by used in investing activities", "net cash provided by (used in) investing activities", "net cash (used in)/generated by investing activities", "net cash generated by investing activities", "cash flows from investing activities")),
    MetricRule("financing_cash_flow_net", StatementType.CASH_FLOW_STATEMENT, ("財務活動によるキャッシュフロー", "財務活動によるキャッシュ・フロー", "財務活動に関するキャッシュフロー", "財務活動に関するキャッシュ・フロー", "財務活動による現金及び現金同等物の増加", "財務活動による現金及び現金同等物の減少", "net cash used in financing activities", "net cash provided by financing activities", "net cash provided by used in financing activities", "net cash provided by (used in) financing activities", "net cash generated by financing activities", "cash flows from financing activities")),
    MetricRule("fx_effect_cash", StatementType.CASH_FLOW_STATEMENT, ("為替変動による現金及び現金同等物への影響額", "effect of exchange rate changes on cash and cash equivalents")),
    MetricRule("cash_equivalents_net_increase", StatementType.CASH_FLOW_STATEMENT, ("現金及び現金同等物の増減額", "現金及び現金同等物純増加", "現金及び現金同等物純減少", "net increase in cash and cash equivalents")),
    MetricRule("cash_equivalents_beginning", StatementType.CASH_FLOW_STATEMENT, ("現金及び現金同等物の期首残高", "現金及び現金同等物期首残高", "cash and cash equivalents at beginning")),
    MetricRule("cash_equivalents_ending", StatementType.CASH_FLOW_STATEMENT, ("現金及び現金同等物の期末残高", "現金及び現金同等物期末残高", "cash and cash equivalents at end")),
...
def find_jp_label_rule(label: str) -> MetricRule | None:
    ...
    matches: list[tuple[int, int, int, MetricRule]] = []
    for index, (key, rule) in enumerate(JP_RULE_BY_LABEL.items()):
        if key and key in normalized and _allow_jp_label_substring(key, rule, normalized):
            matches.append((len(key), rule.priority, -index, rule))
    if not matches:
        return None
    return max(matches)[3]


_JP_EXACT_TOTAL_RULES = {
    "total_assets",
    "current_assets",
    "non_current_assets",
    "total_liabilities",
...
_JP_EXACT_LABEL_KEYS = {
    compact_label("収益"),
    compact_label("法人税等"),
}


def _allow_jp_label_substring(key: str, rule: MetricRule, normalized: str) -> bool:
    if key in _JP_EXACT_LABEL_KEYS:
        return normalized == key
    has_japanese = any("\u3040" <= ch <= "\u30ff" or "\u4e00" <= ch <= "\u9fff" for ch in key)
    if has_japanese and rule.canonical_name in _JP_EXACT_TOTAL_RULES:
        return normalized == key
    return True
