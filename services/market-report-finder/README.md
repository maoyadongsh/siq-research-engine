...
最终问答的上限从文件发现阶段就已确定。本服务在下载前后固定一组后续全链路都会复用的身份和来源信息：市场、官方主体标识、证券代码/CIK/法人编号、报告类型、报告期、披露日期、官方 URL、文件名、下载时间和内容 hash。

- 主体解析与市场选择是两个独立步骤；中文 alias 只能在指定市场内解析，不能跨市场“猜最近似公司”。
- 官方 API/站点返回的 filing/document ID 优先于文件名推断，下载目录只是存储位置，不是事实身份。
- 限速、重试、User-Agent、代理和镜像策略按市场隔离，避免某一市场的请求习惯污染其他官方源。
- 原始文件保持不可变；后续 parser 修正产生新 artifact，不回写官方披露原件。
- 下载结果被 API 通过受控 token 暴露，Agent 和前端不直接读取任意宿主路径。

这一步建立的官方来源链决定了 evidence package 是否能在尽调、审计、合规或客户交付中被第三方复核。
