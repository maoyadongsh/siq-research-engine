...
description: |
  ...
  - Simple web search for company info → use web search directly
  ...
  TOOLS: sessions_spawn (sub-agents), web search, web_fetch
...
"Light" = include in synthesis but don't spawn a dedicated sub-agent. Use web search inline.
...
- Use web search to enrich with real market data; only cite verifiable sources
