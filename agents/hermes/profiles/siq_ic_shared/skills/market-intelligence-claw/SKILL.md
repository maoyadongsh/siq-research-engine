...
5. **Graceful fallback** — if any API fails or is absent, fall back silently to web search and note it in sources
...
If yes: run a lightweight scan using only web search. No paid calls unless approved.
