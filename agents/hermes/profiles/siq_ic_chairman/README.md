...
主席材料必须把 `confirmed / assumed / unknown / disputed` 分层，并保留每项关键结论的委员来源、证据等级、财务计算/勾稽状态和异议。`project_shared` 记忆用于延续历史争议与决策条件，不能把上一轮 IC 结论自动升级为当前事实；新材料、会议补充和委员复核优先。

条件化通过应写成可测试条件、责任人、截止时间和触发后的状态迁移，避免只给“原则同意”式不可审计结论。
