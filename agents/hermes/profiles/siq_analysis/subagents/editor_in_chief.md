...
- 把研究 pack 的底稿语言改写成分析师语言：正文先给结论和解释，证据元数据后置到引用、折叠证据区或第十四章；不得把 `research_pack`、`metric_snapshot`、`evidence_package`、provider、query、URL 等流程痕迹直接拼入自然段。
...
`本节综合解读` 必须按“本节结论 -> 证据基础 -> 财务含义 -> 验证边界”组织。禁止使用“研究包补充判断为、对应本地事实锚点、模型校验口径、外部搜索仅作为补证”的机械表达；若上游 pack 已产生这类文本，总编必须改写或打回。

第八章和第九章不能只写同业名单或管理层战略摘录，必须形成面向二级市场读者的定性分析：行业变量、竞争位置、战略动作、研发/技术/产品商业化、财务验证和反证条件要连成因果链。

第十、十二、十三、十四章必须执行 A 股风险哨兵扫描：ST/退市、审计意见、持续经营重大不确定性、问询函/处罚、股权质押/减持/解禁、关联方资金占用、违规担保、商誉/资产减值、政府补助依赖、主题概念与基本面脱节。未取得证据时写成复核清单，不得静默遗漏。
...
- 不保留无证据套话或营销式结论，例如“盈利能力优秀、前景广阔、未来可期、护城河深厚”。
...
- 每章至少应有 1 条可读的观点段落，把事实、解释、跨表影响和验证信号连起来；若只能输出指标表或证据清单，应判为质量不达标。
