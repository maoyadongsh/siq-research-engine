...
只要回答涉及财报、财务指标、经营分析、风险判断、事实核查、持续跟踪或数据库/Wiki/PDF/SEC HTML/XBRL 解析结果，就必须执行本契约。
...
- 每条引用必须绑定当前完整 ResearchIdentity 和 report_id，禁止从同名公司、其他年份、其他 filing 或其他 parse run 借用引用。
- `pdf_market` 使用 PDF 页码、表格、Markdown 行和 task_id；下列 PDF 规则继续适用。
- `sec_ixbrl` 使用 SEC 官方 source URL，加 section/HTML anchor/xpath 或 XBRL fact id/concept/context/unit。不得为 SEC 报告伪造 PDF 页码或 `/api/pdf_page` 链接。
- 同名 XBRL concept 必须结合 context、期间和 dimensions 消歧；GAAP taxonomy 与公司扩展概念必须显式区分。
- source quality 为 warning、locator 无法解析或身份不一致时，引用状态必须降级或失败，不得静默标记 resolved。
...
- 当前统一解析器为 `/home/maoyd/siq-research-engine/data/hermes/home/profiles/shared/scripts/local_citations.py`。引用出现 `未返回`、只有 `table_index`、或只有 `wiki_metrics/wiki_evidence/wiki_analysis/wiki_semantic` 文件线索时，应按该解析器路径回溯：`company.json -> task_id/report_id -> metrics/evidence/semantic -> reports/<report_id>/document_full.json/pdf_refs.json -> pdf_page/table/md_line`。
