...
正式分析必须从 Research Universe 返回的 market、`company_key`、`report_id` 和完整 ResearchIdentity 开始，随后以服务端解析的 manifest 为权威。客户端不得提交本地路径。`_meta/company_catalog.json` 只由 resolver 在服务端读取，CN 兼容入口可直接从根 catalog 开始：

```text
/home/maoyd/siq-research-engine/data/wiki/_meta/company_catalog.json
```

海外市场使用独立但同构的市场 Wiki 根目录：`data/wiki/<market>/companies/<company_wiki_id>/`。例如港股 HK：
...
正式链必须通过 Research Universe report package resolver 唯一定位；`resolve_company.py` 只用于 CN 旧入口。严禁手写猜测任一市场公司路径。
...
正式链一律 manifest-first：

- `pdf_market`：manifest 声明的全文/指标/证据优先，其次报告级 `report.md`、`document_full.json`、`metrics/normalized_metrics.json`、`qa/source_map.json`，最后才使用公司级 report-specific/latest 兼容入口。
- `sec_ixbrl`：直接读取现有 `parser/document_full.json`、`sections/report_complete.md`、`metrics/financial_data.json`、`metrics/normalized_metrics.json`、`metrics/financial_checks.json`、`qa/source_map.json`、`sections/`、`tables/` 和 `xbrl/`，不得要求或复制 A 股式文件。
- 适配路由由 `source_family/document_format/source_id` 决定，不以 `market == US` 作为唯一条件。ESEF-only 报告在适配器未实现前明确 unavailable。
...
```bash
python3 /home/maoyd/siq-research-engine/data/hermes/home/profiles/shared/scripts/pg_query.py --schema pdf2md --limit 50 --timeout-ms 5000 --sql "<只读 SQL>"
```
