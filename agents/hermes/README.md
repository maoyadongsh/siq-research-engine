...
| OpenShell 灰度运行面 | `siq_analysis` 可由 API 路由到 NVIDIA OpenShell 沙箱 | 支持公司范围自动创建、对话代际、资源池租约/隔离/恢复和 Host 回退；正式生产门禁仍为 `NO_GO` |
...
Host Hermes 仍是默认运行面；`siq_analysis` 可以由 API 控制面按公司范围路由到 NVIDIA OpenShell 灰度沙箱。当前实现没有采用 NemoClaw / NemoHermes，而是保留 SIQ `/v1/runs`、SSE、停止、报告路径、profile 和 Prompt 合同，在 OpenShell 沙箱中运行 BYOC Hermes 镜像。
...
这条路径当前是演示/灰度控制面。它证明 SIQ 已经真实使用 OpenShell 的核心网关、沙箱、Provider、策略、转发和生命周期能力，但正式生产门禁仍为 `NO_GO`，不能把灰度链路存活等同于默认生产切流。
