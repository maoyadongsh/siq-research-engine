...
DEFAULT_LEGACY_ROOT = REPO_ROOT / "data" / "wiki" / "eu_reports"
...
def _target_paths(manifest: dict[str, Any], output_root: Path) -> tuple[Path, Path, str, str]:
    ...
    report_id = manifest.get("report_id") or eu_report_id(manifest.get("fiscal_year"), manifest.get("report_type"), filing_key)
...
def _normalized_manifest(manifest: dict[str, Any], *, company_dir: Path, package_dir: Path, company_wiki_id: str, report_id: str) -> dict[str, Any]:
    normalized = {
        **manifest,
        "market": "EU",
        "report_id": report_id,
...
def migrate_package(source_package: Path, output_root: Path, *, force: bool = False) -> Path:
    ...
    if not isinstance(manifest, dict) or manifest.get("market") != "EU":
        raise ValueError(f"not an EU package: {source_package}")
    ...
    normalized = _normalized_manifest(manifest, company_dir=company_dir, package_dir=target_package, company_wiki_id=company_wiki_id, report_id=report_id)
    write_json(target_package / "manifest.json", normalized)
    write_eu_company_wiki_indexes(output_root, company_dir, normalized, _quality(target_package))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Migrate legacy data/wiki/eu_reports packages into A-share-aligned EU company Wiki layout.")
