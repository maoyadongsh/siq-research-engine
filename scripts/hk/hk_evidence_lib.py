    ...
    if statement_type == StatementType.BALANCE_SHEET:
        if any(
            ...
            for term in (
                ...
                "materialnoncontrollinginterests",
                "noncontrollinginterestsof",
                "summarisedfinancialinformation",
                "summaryfinancialinformation",
                "amountsbeforeintragroupeliminations",
                "beforeintragroupeliminations",
                "majorsubsidiaries",
                "主要財務信息",
                "主要财务信息",
                "分部資料",
...
def _formal_statement_window(text: str) -> tuple[int, int] | None:
    ...
    for pattern in (
        ...
        r"(?im)^#{1,6}\s+notes to financial statements",
        r"(?im)^#{1,6}\s+財務報表附註",
        r"(?im)^#{1,6}\s+财务报表附注",
        r"(?im)^#{1,6}\s+合併財務報表附註",
        r"(?im)^#{1,6}\s+合并财务报表附注",
