#!/bin/bash -p
# Execute one owner-scoped SIQ OpenShell pool lifecycle command.
...
readonly COMMAND="${1:-}"
if [[ "$COMMAND" != start && "$COMMAND" != migrate-legacy && "$COMMAND" != stop && "$COMMAND" != status && "$COMMAND" != probe && "$COMMAND" != rollback ]]; then
  printf '%s\n' 'Usage: run_siq_analysis_pool_lifecycle.sh <start|migrate-legacy|stop|status|probe|rollback> [arguments...]' >&2
  exit 2
fi
shift
...
if [[ "$COMMAND" == start || "$COMMAND" == migrate-legacy || "$COMMAND" == stop || "$COMMAND" == rollback ]]; then
...
exec /usr/bin/env -i "${environment[@]}" \
  /usr/bin/python3 -I -B "$SCRIPT_DIR/siq_analysis_pool_lifecycle.py" "$COMMAND" "$@"
