...
# NOT_PRODUCTION：真实 siq_analysis + 已配置 MiniMax Provider 的隔离观察 PoC。
...
# NOT_PRODUCTION：真实公司 company.json + 正式七挂载/叶子策略的宽松业务 pilot。
...
# NOT_PRODUCTION_CANARY 资源池槽位：端口 reservation/token 由 owner-only 管理器内部处理。
# status/stop 从注册表读取精确槽位 endpoint，不会影响 28651 legacy canary。
...
# PostgreSQL 读取身份：默认只输出 plan；apply/rollback 需要精确确认
...
# start_all.sh 默认自动管理并强制严格请求身份；出网 broker 不继承数据库或模型凭据。
...
# Provider 默认 dry-run；apply 不会打印凭据值
...
`run_siq_analysis_fallback_drill.py` 必须在正常 A/B 已 GO 且没有活跃正式 transaction 时
运行。它使用独立 `fallback-*` transaction，并在宿主 `8004` 上临时建立仅 Docker bridge 可达的
503 stub；这只验证当前主 Provider 到既有回退路径的真实切换和遥测，不启用或
...
诊断证据，publisher、completion、回退和正式业务回执不得把它们当作新口径结果复用。
...
Provider、`8007` Nemotron 可用回退、`/v1/runs` 创建/SSE/停止和终端工具链路可行。Hermes 运行态 home
...
Search/Query/Get/Describe broker 可达。旧布尔自证会被拒绝。协议发现通过不代表模型推理、embedding 生成、回退或
