#!/bin/bash -p
# Execute one isolated NOT_PRODUCTION canary lifecycle command.
...
readonly COMMAND="${1:-}"
if [[ "$COMMAND" != start && "$COMMAND" != stop && "$COMMAND" != status && "$COMMAND" != probe && "$COMMAND" != rollback ]]; then
  printf '%s\n' 'Usage: run_siq_analysis_canary_lifecycle.sh <start|stop|status|probe|rollback> [arguments...]' >&2
  exit 2
fi
shift
...
if [[ "$COMMAND" == start || "$COMMAND" == stop || "$COMMAND" == rollback ]]; then
...
exec /usr/bin/env -i "${environment[@]}" \
  /usr/bin/python3 -I -B "$SCRIPT_DIR/siq_analysis_canary.py" "$COMMAND" "$@"
