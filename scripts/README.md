...
R1 后序 profile 仍遵守严格串行门禁。若要单独冒烟 `siq_ic_finance_auditor` 等后序角色，可加 `--seed-prior-reports`，脚本只会在临时 package 中写入前序 agent 的 synthetic R1 报告和 `submitted_agents`，不会改真实项目包。
