...
`scripts/` 保存 SIQ Research Engine 的运维、维护、Hermes 辅助和向量入库脚本。这里放可重复执行的薄脚本，不放应用源码、运行态数据、模型权重或生成报告。
...
| `scripts/maintenance` | 数据集生成、离线整理、批量维护等任务 |
...
| 入口 | 用途 |
| --- | --- |
| `start_all.sh` | 一键启动 Web、API、PDF 解析、公告下载和 Hermes 网关 |
