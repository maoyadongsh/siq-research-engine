...
该脚本聚合 Agent runtime streaming owner、PDF parser source/artifact、Web Node unit、`npm run check:frontend` 和提交前检查。其中 `git diff --check` 是失败门禁，`git status --short` 是收尾 review 输出。它是当前架构优化收口门禁，不替代 `scripts/check_all.sh` 的全量基础检查；`scripts/check_all.sh` 对齐 README 的合并前基础门禁，用于更重的全仓验证。
